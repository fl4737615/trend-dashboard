import os
import io  # Added for XML parsing
import nltk
import heapq
import logging
import pandas as pd
...
from defusedxml.lxml import parse as defused_parse  # Corrected import
...
class TrendRadar:
    ...
    def _get_research_pulse(self):
        try:
            ...
            tree = defused_parse(io.BytesIO(response.content))  # Secure parsing
            return [entry.findtext('title') for entry in tree.findall('.//entry')[:30]]
        except Exception as e:
            logging.warning(f"arXiv failed: {str(e)}")
            return []

    # Other methods (_get_news_entities, _get_stock_signals) remain unchanged

# ========== UI CODE UNCHANGED ========== #
# ... (identical to previous UI implementation)
